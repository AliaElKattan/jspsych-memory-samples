...
jsPsych.plugins["psychophysics"] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      dash_length: {
        type: jsPsych.plugins.parameterType.INT,
        pretty_name: 'Dash length',
        default: null,
        description: 'Length of sub-line in a dashed line. The gap is half this length by default.'
      },
      dash_gap: {
        type: jsPsych.plugins.parameterType.INT,
        pretty_name: 'Dash length',
        default: null,
        description: 'Length of gaps in a dashed line.'
      },
      response_ends_trial: {
        type: jsPsych.plugins.parameterType.BOOL,
  ...
  plugin.trial = function(display_element, trial) {
    ...
    function set_dashed_line(stim){
      ...
      if (typeof stim.line_color === 'undefined') stim.line_color = '#000000';
      if (typeof stim.dash_length === 'undefined') stim.dash_length = 10;
      if (typeof stim.dash_gap === 'undefined') stim.dash_gap = (stim.dash_length / 2);
    }
    ...
    function set_triangle(stim){
      common_set(stim);
      if (typeof stim.width === 'undefined' && typeof stim.height === 'undefined') alert('You have to specify the width or height of the triangle.');
      if (typeof stim.fill_color === 'undefined') alert('You have to specify the fill_color property for the triangle.');
    }

    function set_diamond(stim){
      common_set(stim);
      if (typeof stim.height === 'undefined' && typeof stim.width === 'undefined') alert('You have to specify the height or the width of the diamond.');
      if (typeof stim.fill_color === 'undefined') alert('You have to specify the fill_color property for the diamond.');
    }

    function set_parallel(stim){
      common_set(stim);
      if (typeof stim.height === 'undefined' && typeof stim.width === 'undefined') alert('You have to specify the height or the width of the parallelogram.');
      if (typeof stim.fill_color === 'undefined') alert('You have to specify the fill_color property for the parallelogram.');
    }
    ...
    const present_functions = {
      ...
      cross: present_cross,
      sound: present_sound,
      triangle: present_triangle,
      diamond: present_diamond,
      parallel: present_parallel,
      dashed_line: present_dashed_line
    }
    ...
    function present_dashed_line(stim){
      // common

      if (typeof stim.dash_length !== 'undefined') stim.dash_gap = (stim.dash_length/3);
      ctx.beginPath();            
      ctx.setLineDash([stim.dash_length, stim.dash_gap])
      ctx.lineWidth = stim.line_width;
      ctx.lineJoin = stim.lineJoin;
      ctx.miterLimit = stim.miterLimit;
      //
    ...
    function present_triangle(stim){
      // common
      ctx.lineWidth = stim.line_width;
      ctx.lineJoin = stim.lineJoin;
      ctx.miterLimit = stim.miterLimit;
      ...
      ctx.beginPath(); 
      ctx.moveTo(stim.currentX,stim.currentY);

      //if we only have width or height, make an equilateral triangle
      if(typeof stim.width === 'undefined') stim.width = (stim.height/Math.sqrt(3))*2;
      if(typeof stim.height === 'undefined') stim.height = stim.width*(Math.sqrt(3)/2);

      if (typeof stim.width !== 'undefined' && typeof stim.height !== 'undefined') { 
      ctx.lineTo(stim.currentX+(stim.width/2),stim.currentY+stim.height);
      ctx.lineTo(stim.currentX-(stim.width/2),stim.currentY+stim.height);
      }
    ...
    function present_parallel(stim){
      // common

      // if (typeof stim.width === 'undefined') {
      //   stim.width = stim.height*2;
      // }
      ...
      ctx.lineTo(stim.currentX+(stim.width),stim.currentY);
      ctx.lineTo(stim.currentX+(stim.width/2),stim.currentY+stim.height);
      ctx.lineTo(stim.currentX-(stim.width/2),stim.currentY+stim.height);
      ...
      ctx.lineTo(stim.currentX+(stim.width/2),stim.currentY+stim.height);
      ctx.lineTo(stim.currentX,stim.currentY+((stim.height)*2));
      ctx.lineTo(stim.currentX-(stim.width/2),stim.currentY+stim.height);
    ...
    var after_response = function(info) {
      ...
      //xdisplay_element.querySelector('#jspsych-html-keyboard-response-stimulus').className += ' responded';

      // only record the first response


      var x_axis = info.clickX - trial.canvas_width/2;
      var y_axis = trial.canvas_height/2 - info.clickY;
        //calculate distance from 0,0 point to x,y point
      var distance = Math.sqrt(Math.pow(Math.abs(x_axis),2) + Math.pow(Math.abs(y_axis),2));

      var radius = trial.stimuli[0].radius;

      //assume canvas_width/2 = radius
      var click_margin = Math.abs(radius - distance);

      console.log(click_margin);

      if (response.key == null) {
        response = info;
      }

      if (trial.response_ends_trial && click_margin < 10) {
        end_trial();
      }
    };
