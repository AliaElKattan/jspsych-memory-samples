...
jsPsych.plugins["psychophysics"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    function set_equi_tri(stim){
      common_set(stim);
      if (typeof stim.height === 'undefined') alert('You have to specify the height of the equilateral triangle.');
      if (typeof stim.fill_color === 'undefined') alert('You have to specify the fill_color property for the equi triangle.');
    }

    function set_iso_tri(stim){
      common_set(stim);
      if (typeof stim.height === 'undefined') alert('You have to specify the height of the isosceles triangle.');
      if (typeof stim.fill_color === 'undefined') alert('You have to specify the fill_color property for the iso triangle.');
    }

    function set_diamond(stim){
      common_set(stim);
      if (typeof stim.height === 'undefined') alert('You have to specify the height of the diamond.');
    ...
    function set_parallel(stim){
      common_set(stim);
      if (typeof stim.height === 'undefined') alert('You have to specify the height of the parallelogram.');
    ...
    const present_functions = {
      ...
      cross: present_cross,
      sound: present_sound,
      equi_tri: present_equi_tri,
      iso_tri: present_iso_tri,
    ...
    function present_equi_tri(stim){
      // common
      ctx.lineWidth = stim.line_width;
      ctx.lineJoin = stim.lineJoin;
      ctx.miterLimit = stim.miterLimit;
      ...
      ctx.lineTo(stim.currentX+(stim.height/Math.sqrt(3)),stim.currentY+stim.height);
      ctx.lineTo(stim.currentX-(stim.height/Math.sqrt(3)),stim.currentY+stim.height);
      ctx.closePath();    
    ...
    function present_iso_tri(stim){
      ...
      ctx.moveTo(stim.currentX,stim.currentY);
      ctx.lineTo(stim.currentX+(stim.width),stim.currentY+stim.height);
      ctx.lineTo(stim.currentX-(stim.width),stim.currentY+stim.height);
